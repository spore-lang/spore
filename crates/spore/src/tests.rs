...
use crate::exec::{create_project, exec_check, exec_format, exec_test, is_valid_type};
use crate::report::hole_graph_update;
use crate::target::{
...
fn test_resolve_sp_targets_empty_paths_rejects_invalid_source_roots() {
    ...
    let error = resolve_sp_targets(&[], dir).expect_err("invalid source-roots should be surfaced");
    assert!(
