...
| **有界代价** | 编译器静态验证 `cost [compute, alloc, io, parallel]` | 性能可预测性 |
...
Capability Narrowing     ← 子任务能力集 ⊆ 父任务能力集（Spore 独有）
       +
Cost Budgets             ← 固定顺序四维代价向量（Spore 独有）
```
...
| Spore 子系统 | 与并发模型的交互点 |
|-------------|-------------------|
| 签名语法 | `uses [Spawn, ...]` 声明并发能力；`cost [compute, alloc, io, parallel]` 固定顺序声明总代价 |
...
-- 提供 handler
handler token_bucket_handler(rate: I32, per: Duration) for RateLimit {
    fn acquire_permit() -> () ! RateLimitExceeded {
        -- token bucket 实现
...
```spore
fn producer(tx: Sender[I32]) -> () ! ChannelClosed
uses [ChanSend[I32]]
{
...
fn consumer(rx: Receiver[I32]) -> List[I32] ! ChannelClosed
uses [ChanRecv[I32]]
{
...
uses [ChanRecv[Command], ChanRecv[Event], ChanRecv[()], Spawn]
{
    select {
        cmd from commands => {
            execute(cmd)
...
- **代价可分析**: 结构化的 task 树在编译时可枚举——
  编译器需要此属性来验证 `cost [compute, alloc, io, parallel]` 约束。
...
- **编译器更擅长**: 编译器能遍历完整 task 树，计算峰值 lane 需求，
  远比人类手动推算准确。
- **签名层面约束**: 函数签名中可声明 `cost [compute, alloc, io, parallel]`，编译器验证推断的 lane 数是否落在该四维预算内。
...
```spore
trait HttpHandler = [Spawn, NetRead, NetWrite, DbRead, Clock]
