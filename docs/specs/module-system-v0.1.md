...
### 7.1 What Is a Platform?

A **Platform** is the only code in a Spore application that can perform real IO. It provides all capabilities that pure packages declare as requirements.

```
┌────────────────────────────────────────────┐
│                 Platform                    │
│  (provides: FileRead, FileWrite, NetRead,  │
│   NetWrite, Clock, Random, Stdout, ...)    │
├────────────────────────────────────────────┤
│              Application                   │
│  main.spore (entry point)                  │
│  uses capabilities provided by Platform    │
├────────────────────────────────────────────┤
│          Pure Packages                     │
│  billing-lib, json-parser, etc.            │
│  declare capability needs but cannot       │
│  perform IO themselves                     │
└────────────────────────────────────────────┘
```

### 7.2 Platform in `spore.toml`

An application declares its platform in its manifest:

```toml
# spore.toml (application)
[package]
...
# Platform provides these capabilities:
# FileRead, FileWrite, NetRead, NetWrite, Stdout, Stderr,
# Stdin, Clock, Random, EnvVar, ProcessSpawn

[dependencies]
billing-lib = { version = "1.2.0", source = "https://packages.spore-lang.org/billing-lib/1.2.0@sha256:..." }
json-parser = { version = "0.8.0", source = "https://packages.spore-lang.org/json-parser/0.8.0@sha256:..." }
```

### 7.3 Pure Package Manifest

A pure package does **not** declare a platform. It declares capability requirements:

```toml
# spore.toml (pure package)
[package]
name = "billing-lib"
version = "1.2.0"
type = "package"
...
# These are abstract capabilities. The application's Platform
...
### 7.4 Platform Capability Mapping

The platform maps abstract capabilities to concrete implementations:

```toml
# Inside the platform's configuration
[capability_providers]
PaymentGateway = "platform.io.http"      # provided via HTTP client
AuditLog       = "platform.io.file"      # provided via file writer
FileRead       = "platform.io.fs.read"
FileWrite      = "platform.io.fs.write"
NetRead        = "platform.io.net.read"
NetWrite       = "platform.io.net.write"
Stdout         = "platform.io.console.stdout"
Clock          = "platform.io.time.clock"
Random         = "platform.io.random"
```

### 7.5 A Minimal Platform Example

```spore
-- platform/src/main.spore
    ...
    uses [RawSyscall]       -- only platforms have RawSyscall
{
    ...
    syscall.write(1, message.as_bytes())
}
    ...
    uses [RawSyscall]
{
    ...
    let content = syscall.read_all(fd)
    syscall.close(fd)
    content
}
...
The Platform system enforces a strict security boundary:

1. **Pure packages cannot perform IO**. They can only declare capability requirements.
2. **The platform is the only code with `RawSyscall`**. No user package can access raw system calls.
3. **Capabilities are explicitly declared**. An Agent or auditor can inspect any function's `uses` clause to know exactly what it can do.
4. **Supply-chain safety**: A downloaded package declaring `uses [Compute]` is provably pure — it cannot access the filesystem, network, or any other IO primitive.
