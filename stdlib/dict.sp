...
fn dict_is_empty[K, V](d: List[Pair[K, V]]) -> Bool cost [4, 0, 0, 0]
spec {
...
@unbounded
fn dict_get_str[V](d: List[Pair[Str, V]], key: Str) -> Option[V] cost [O(d), 1, 0, 0]
spec {
...
@unbounded
fn dict_remove[K, V](d: List[Pair[K, V]], key: K) -> List[Pair[K, V]] cost [O(d), O(d), 0, 0]
spec {
...
@unbounded
fn dict_contains_key[K, V](d: List[Pair[K, V]], key: K) -> Bool cost [O(d), 0, 0, 0] {
    match d {
        [] => false,
...
@unbounded
fn dict_keys[K, V](d: List[Pair[K, V]]) -> List[K] cost [O(d), O(d), 0, 0] { map(d, |entry: Pair[K, V]| entry.first) }

@unbounded
fn dict_values[K, V](d: List[Pair[K, V]]) -> List[V] cost [O(d), O(d), 0, 0] { map(d, |entry: Pair[K, V]| entry.second) }
