...
fn map_option[T, U](opt: Option[T], f: (T) -> U) -> Option[U] cost [8, 0, 0, 0] {
    match opt {
        Some(v) => Some(f(v)),
        None => None,
    }
...
fn and_then[T, U](opt: Option[T], f: (T) -> Option[U]) -> Option[U] cost [5, 0, 0, 0] {
    match opt {
        Some(v) => f(v),
        None => None,
    }
...
fn or_else[T](opt: Option[T], f: () -> Option[T]) -> Option[T] cost [5, 0, 0, 0] {
    match opt {
        Some(v) => Some(v),
        None => f(),
    }
...
fn map_result[T, U, E](res: Result[T, E], f: (T) -> U) -> Result[U, E] cost [8, 0, 0, 0] {
    match res {
        Ok(v) => Ok(f(v)),
        Err(e) => Err(e),
    }
...
fn and_then_result[T, U, E](res: Result[T, E], f: (T) -> Result[U, E]) -> Result[U, E] cost [5, 0, 0, 0] {
    match res {
        Ok(v) => f(v),
        Err(e) => Err(e),
    }
...
fn map_err[T, E, F](res: Result[T, E], f: (E) -> F) -> Result[T, F] cost [8, 0, 0, 0] {
    match res {
        Ok(v) => Ok(v),
        Err(e) => Err(f(e)),
    }
...
fn or_else_result[T, E, F](res: Result[T, E], f: (E) -> Result[T, F]) -> Result[T, F] cost [5, 0, 0, 0] {
    match res {
        Ok(v) => Ok(v),
        Err(e) => f(e),
    }
...
fn flatten_result[T, E](res: Result[Result[T, E], E]) -> Result[T, E] cost [5, 0, 0, 0] {
    match res {
        Ok(inner) => inner,
        Err(e) => Err(e),
    }
